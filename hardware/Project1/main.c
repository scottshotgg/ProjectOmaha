/*
 * main source code for TMS320F28377S cpu.
 *
 * White noise is generated
 *
 *
 * Created by Matt Kramer and Scott Gaydos
 * Summer and Fall 2015, University of Texas at Dallas
 *
 *
 *
 */
...
int						idNumber								= 107;
int						idNumberRecieved						= 107;
int						recievedData							= 0;
int						commandRecieved							= 0;
int						transmissionAmountInBytes				= 2;
int						amountofDataRecievedInBytes				= 0;
...
void initialize(){
	...
	ConfigCpuTimer(&CpuTimer0, 200, 500000/(SAMPLERATE));	 	//DAC will output at nyquist rate of twice the samplerate. 500ms/samplerate = 1sec/(2*samplerate)
	ConfigCpuTimer(&CpuTimer1, 200, 1000000/(ADCSAMPLERATE));
...
__interrupt void cpu_timer0_isr(void)
{
	...
	filteredArray[j] = hnd_firFP->output;
	toDac[j] = ((filteredArray[j])/activeNorm + 2048);
	if(toDac[j] > maxValue)
		maxValue = toDac[j];
	if(toDac[j] < minValue)
		minValue = toDac[j];
	DacaRegs.DACVALS.bit.DACVALS = toDac[j];					//output filtered "pink noise"
...
__interrupt void sciaRxFifoIsr(void)
{
	// Need to determine ID number before this, it is hardcoded to 107 at the top
	recievedData = SciaRegs.SCIRXBUF.all;

	if(!idNumberRecieved)
	{
		idNumberRecieved = recievedData;
	}
	else
	{
		if(!transmissionAmountInBytes)
		{
			transmissionAmountInBytes = recievedData;
		}
		else
		{
			amountofDataRecievedInBytes++;

			if(!commandRecieved)
			{
				commandRecieved = recievedData;
			}

			if(idNumberRecieved == idNumber)
			{
				switch(commandRecieved)
				{
					...
					case 'm':
						if(amountofDataRecievedInBytes == 2)
						{
							volumeLevel = recievedData;
						}
						break;

					case 'l':
						SciaRegs.SCITXBUF.all = GPIO_ReadPin(13);
						break;

					case 'c':
						SciaRegs.SCIFFRX.bit.RXFFIL = transmissionAmountInBytes - 1;
						// Need to do something about the next interrupt with data

						// Piece of algorithm
							// Things like this may warrant and ending byte
							// Might need a subprotocol for sending coefficients to tell it how many
							// or we can send every coefficient since there aren't that many
						// End piece
			...
			if(amountofDataRecievedInBytes == transmissionAmountInBytes)
			{
				amountofDataRecievedInBytes = 0;
				//transmissionAmountInBytes = 0;		// Uncomment this to run full algorithm
				//idNumberRecieved = 0;					// Uncomment this to run full algorithm
				commandRecieved = 0;
				//need an if statement here
				//coefficient = ((SciaRegs.SCIRXBUF.all << 24) | (SciaRegs.SCIRXBUF.all << 16) | (SciaRegs.SCIRXBUF.all << 8) | SciaRegs.SCIRXBUF.all);
				// I think this will work and take them all out of the FIFO
				// Might be more efficient to make a loop and use some....... MATH
			}
			// Check to reset idNumberRecieved
		}
	}

	/*for(a = 0; a < 16; a++){
		receivedChar[a]=SciaRegs.SCIRXBUF.all;			// This needs to be changed to include a variable that is set by the command for the for loop
		SciaRegs.SCITXBUF.all = receivedChar[a];		// This should also have an if else compare to ID in which this for loop is expanded. Remember roll over
	}*/
