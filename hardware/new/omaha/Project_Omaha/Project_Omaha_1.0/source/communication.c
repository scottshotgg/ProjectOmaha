...
void addressCheck(){ //read 1st byte recieved and determine who the message is for
	...
	if(receivedChar[0] == 108){ //message is intended for the current unit.

p75		readCommand();//interpret the message. After this the message will be terminated.
	}

	else{ //message needs to be transmitted along

		GpioDataRegs.GPADAT.bit.GPIO3 = 1;  //used for SCIB TX. Turn on to transmit message //GPIO 9 used for scia!!!
